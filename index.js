...
class NodeID3 {
    ...
    getTagsFromBuffer(filebuffer) {
        ...
        frames.forEach((frame) => {
            ...
            if (frame.name[0] === 'T' && frame.name !== 'TXXX') {
                //  Decode body
                let decoded;
                decoded = iconv.decode(frame.body.slice(1), this.getEncodingName(frame.body)).replace(/\0/g, multiValueSplitter);
    ...
    */
    ...
    getTagHeaderSize(data) {
        ...
        const hSize = new Buffer([data[framePosition + 6], data[framePosition + 7], data[framePosition + 8], data[framePosition + 9]]);
