...
export class NodeID3 {
  ...
  public update(tags: any, filebuffer: Buffer | string, fn: Function) {
    ...
    Object.keys(tags).map((tagKey) => {
      ...
      if (TFrames[tagKey]) {
        ...
        Object.keys(this.SpecialFrames)
  ...
  public getFramesFromID3Body(ID3FrameBody: Buffer, id3Version: string) {
    const frames = [];
    ...
    while (currentPosition < ID3FrameBody.length && ID3FrameBody[currentPosition] !== 0x00) {
      ...
      const bodyFrameSize = this.getFrameSize(bodyFrameHeader, 4, id3Version === TagVersion.v24);
  ...
  private replace(buf: Buffer, findStr: any, replaceStr: Uint8Array): Buffer {
   ...
   **  frame   => Buffer
   */
  public readCommentFrame(frame: Buffer, unsynchronized: boolean) {
    let tags = {};
    if (!frame) {
      return tags;
    }
